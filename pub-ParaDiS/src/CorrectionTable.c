 ...
 */
                ...
                if (fscanf(fp,"%d%d%d", &ctab_norder, &ctab_uorder, &ctab_levels) !=3){
                   fprintf(stderr, "Expected at least 3 number as input\n");
                   exit(1);
                }
                break;
 ...
 */
        ...
        for (i = 0; i < n*k; i++) {
            for (j = 0; j < 6; j++) {
                if( fscanf(fp, "%lf", &correctionTbl[i*6+j])!=1){
                        fprintf( stderr, "Expected 1 number as input\n");
                   exit(1);
