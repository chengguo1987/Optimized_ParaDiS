 ...
 */
  #pragma omp parallel
 {
        ...
        FMLayer_t *threadLayer;
        GetThreadIterationIndices(param->fmNumLayers-1, &threadID,
                                      &threadIterStart, &threadIterEnd);

        #pragma omp for

         for (layerID = threadIterStart; layerID < threadIterEnd; layerID++) {
 ...
 *          neighbors.
 */
            threadLayer = &home->fmLayer[layerID+1];

            bMin = threadLayer->ownedMin;
            bMax = threadLayer->ownedMax;
 ...
 */
                        cellID = EncodeFMCellIndex(threadLayer->lDim,cx,cy,cz);
                        cell = LookupFMCell(threadLayer->cellTable, cellID);
 ...
 */
                        for (itx = tMin[X]; itx <= tMax[X]; itx++) {
                            tx = itx % threadLayer->lDim[X];
                            tx = GETPBCINDEX(tx, threadLayer->lDim[X]);
                            for (ity = tMin[Y]; ity <= tMax[Y]; ity++) {
                                ty = ity % threadLayer->lDim[Y];
                                ty = GETPBCINDEX(ty, threadLayer->lDim[Y]);
                                for (itz = tMin[Z]; itz <= tMax[Z]; itz++) {
                                    tz = itz % threadLayer->lDim[Z];
                                    tz = GETPBCINDEX(tz, threadLayer->lDim[Z]);
 ...
 */
                                    nCellID = EncodeFMCellIndex(threadLayer->lDim,tx,ty,tz);
                                    nCell = LookupFMCell(threadLayer->cellTable,
                                                         nCellID);

                                    R[X] = (threadLayer->cellSize[X] * (cx-itx));
                                    R[Y] = (threadLayer->cellSize[Y] * (cy-ity));
                                    R[Z] = (threadLayer->cellSize[Z] * (cz-itz));
