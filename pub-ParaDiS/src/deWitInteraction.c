 ...
 */
	if (home->myDomain == 0) {
		...
		//fscanf(fp,"%d %d %d",&NIMGX,&NIMGY,&NIMGZ);
		 if( fscanf(fp,"%d %d %d",&NX,&NY,&NZ) !=3){
                 fprintf( stderr, "Expected 3 numbers as input\n");
     exit(1);
                }
                if( fscanf(fp,"%le %le %le",&Lx,&Ly,&Lz) !=3){
                 fprintf( stderr, "Expected 3 numbers as input\n");
     exit(1);
                }
                if( fscanf(fp,"%d %d %d",&NIMGX,&NIMGY,&NIMGZ) !=3){
                 fprintf( stderr, "Expected 3 numbers as input\n");
     exit(1);
                }
		...
		for(m=0;m<10;m++) {
			...
			if(fscanf(fp,"%le\n", &RIJMTABLE[i*Offset1+j*Offset2+k*Offset3+m]) !=1){
                  fprintf( stderr, "Expected 1 numbers as input\n");
      exit(1);
      }
		}
 ...
 */
	if (home->myDomain == 0) {
     ...
     exit(1);
		...
		for(k=0;k<NZ;k++)
		for(m=0;m<10;m++) {
			if (fscanf(fp,"%le\n", &RIJMPBCTABLE[i*Offset1+j*Offset2+k*Offset3+m])!=1){
			 fprintf( stderr, "Expected at least 1 number as input\n");
        exit(1);
