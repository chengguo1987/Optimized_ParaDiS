...
void Vector3Matrix33Mult(real8 vec[3], real8 mat[3][3], real8 result[3])
{
        ...
        */
        double tmp[4] = {0.0, 0.0, 0.0, 0.0};
	      __m256d v0= _mm256_set_pd(vec[0], vec[0], vec[0], vec[0]);
	      __m256d v1= _mm256_set_pd(vec[1], vec[1], vec[1], vec[1]);
	      __m256d v2= _mm256_set_pd(vec[2], vec[2], vec[2], vec[2]);

	      __m256d rows0 = _mm256_loadu_pd(mat[0]);
	      __m256d rows1 = _mm256_loadu_pd(mat[1]);
	      __m256d rows2 = _mm256_loadu_pd(mat[2]);

	      __m256d prod0 = _mm256_mul_pd(v0, rows0);
	      __m256d prod1 = _mm256_mul_pd(v1, rows1);
	      __m256d prod2 = _mm256_mul_pd(v2, rows2);

	      __m256d   res = _mm256_add_pd(_mm256_add_pd(prod0, prod1), prod2);
...
void Matrix33Vector3Multiply(real8 A[3][3], real8 x[3], real8 y[3])
{
        ...
        */ 
        ...
        __m256d res = _mm256_add_pd(_mm256_mul_pd(A0, x0), _mm256_mul_pd(A1, x1));
...
void Matrix43Vector3Multiply(real8 A[4][3], real8 x[3], real8 y[4])
{
        ...
        */
        double tmp[4];
		    __m256d A0 = _mm256_set_pd(A[3][0], A[2][0], A[1][0], A[0][0]);
		    __m256d A1 = _mm256_set_pd(A[3][1], A[2][1], A[1][1], A[0][1]);
		    __m256d A2 = _mm256_set_pd(A[3][2], A[2][2], A[1][2], A[0][2]);
		    __m256d x0 = _mm256_set_pd(x[0], x[0], x[0], x[0]);
		    __m256d x1 = _mm256_set_pd(x[1], x[1], x[1], x[1]);
		    __m256d x2 = _mm256_set_pd(x[2], x[2], x[2], x[2]);
		    __m256d res = _mm256_add_pd(_mm256_mul_pd(A0, x0), _mm256_mul_pd(A1, x1));
			    res = _mm256_add_pd(res, _mm256_mul_pd(A2, x2));
		    _mm256_storeu_pd(tmp, res);
