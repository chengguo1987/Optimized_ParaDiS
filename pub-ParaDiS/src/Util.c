...
void cross(real8 a[3], real8 b[3], real8 c[3])
{
  ...
  ***************************/
  ...
  _mm256_zeroall();
	...
	__m256d ta, tb, tc, ea, eb;
	double tmp[4] = {0.0, 0.0, 0.0, 0.0};
	
	// set to va[1][2][0][3] , vb[2][0][1][3]
	ea = _mm256_set_pd(0, a[0], a[2], a[1]);
	eb = _mm256_set_pd(0, b[1], b[0], b[2]);

	// set to va[2][0][1][3] , vb[1][2][0][3]
	ta = _mm256_set_pd(0, a[1], a[0], a[2]);
	tb = _mm256_set_pd(0, b[0], b[2], b[1]);


    tc= _mm256_sub_pd(_mm256_mul_pd(ea, eb) , _mm256_mul_pd( ta , tb ) );

	_mm256_storeu_pd(tmp,tc);
	c[0] = tmp[0];
	c[1] = tmp[1];
	c[2] = tmp[2];
}
